...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => ''.e($tutor->user->name).' — TemanLes']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => ''.e($tutor->user->name).' — TemanLes']); ?>

    <a href="<?php echo e(route('marketplace')); ?>" class="inline-flex items-center gap-1.5 text-sm text-ink-muted hover:text-ink mb-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-board rounded px-1">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-line rounded-2xl p-6">
                <div class="flex items-start gap-4 mb-5">
                    ...
                    <div class="min-w-0">
                        <h1 class="font-display text-2xl font-semibold text-ink"><?php echo e($tutor->user->name); ?></h1>
                        <p class="text-sm text-ink-muted"><?php echo e($tutor->headline); ?></p>
                ...
                <div class="flex flex-wrap gap-2 chalk-divider pt-4">
                    <span class="text-xs bg-board/8 text-board px-2.5 py-1 rounded-full font-medium">
                        <?php echo e($tutor->experience_years); ?> tahun mengajar
                    </span>
                    <?php if($tutor->education): ?>
                        <span class="text-xs bg-board/8 text-board px-2.5 py-1 rounded-full font-medium"><?php echo e($tutor->education); ?></span>
                    <?php endif; ?>
                    <span class="text-xs bg-board/8 text-board px-2.5 py-1 rounded-full font-medium">
            ...
            <div class="bg-white border border-line rounded-2xl overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <div id="tab-tentang" class="tab-content block">
                        ...
                        <?php if($tutor->bio): ?>
                            <p class="text-sm text-ink-muted leading-relaxed whitespace-pre-line"><?php echo e($tutor->bio); ?></p>
                        <?php else: ?>
                    ...
                    <div id="tab-pelajaran" class="tab-content hidden">
                        ...
                        <?php if($tutor->tutorSubjects->isNotEmpty()): ?>
                            <div class="divide-y divide-line">
                                <?php $__currentLoopData = $tutor->tutorSubjects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ts): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                                        <span class="text-sm text-ink"><?php echo e($ts->subject->name); ?> <span class="text-ink-muted">— <?php echo e($ts->level); ?></span></span>
                                        <span class="text-sm font-semibold text-ink">Rp<?php echo e(number_format($ts->price_per_hour, 0, ',', '.')); ?>/jam</span>
                                    </div>
                    ...
                    <div id="tab-jadwal" class="tab-content hidden space-y-6">
                        <div>
                            ...
                            <?php if($tutor->availabilities->isNotEmpty()): ?>
                                <div class="flex flex-wrap gap-2">
                                    <?php $hari = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu']; ?>
                                    <?php $__currentLoopData = $tutor->availabilities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <?php if(in_array($tutor->teaching_mode, ['offline', 'both']) && $tutor->default_address): ?>
                            <div class="chalk-divider pt-6">
                                <h2 class="font-semibold text-ink mb-2">Area Mengajar Tatap Muka</h2>
                                <p class="text-sm text-ink-muted"><?php echo e($tutor->default_address); ?></p>
                    ...
                    <div id="tab-ulasan" class="tab-content hidden">
                        ...
                        <?php if($tutor->reviews->isNotEmpty()): ?>
                            <div class="space-y-5">
                                <?php $__currentLoopData = $tutor->reviews; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $review): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="chalk-divider pt-4 first:pt-0 first:border-t-0">
                                        <div class="flex items-center justify-between mb-1">
                                            <span class="text-sm font-medium text-ink"><?php echo e(explode(' ', $review->student->name)[0]); ?></span>
                                            <div class="flex items-center gap-1">
                                                <?php for($i = 1; $i <= 5; $i++): ?>
                                                    <svg class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="<?php echo e($i <= $review->rating ? '#5DF8D8' : '#E1DED4'); ?>" stroke="#093C5D" stroke-width="0.5" aria-hidden="true"><path d="M10 1.5l2.6 5.6 6.1.6-4.6 4.1 1.3 6-5.4-3.1-5.4 3.1 1.3-6-4.6-4.1 6.1-.6z"/></svg>
                                                <?php endfor; ?>
                                            </div>
                                        </div>
                                        <?php if($review->comment): ?>
                                            <p class="text-sm text-ink-muted"><?php echo e($review->comment); ?></p>
                                        <?php endif; ?>
                                        <p class="text-xs text-ink-muted mt-1"><?php echo e($review->created_at->translatedFormat('d M Y')); ?></p>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        <?php else: ?>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white border border-line rounded-2xl p-6 sticky top-24">
                ...
                <p class="font-display text-2xl font-semibold text-ink mb-5">
                    <?php echo e($minPrice ? 'Rp' . number_format($minPrice, 0, ',', '.') . '/jam' : '—'); ?>

                </p>

                <?php if($tutor->tutorSubjects->isNotEmpty()): ?>
                    <a href="<?php echo e(route('booking.step1', $tutor)); ?>" class="block text-center bg-board text-white rounded-lg py-3.5 font-medium hover:bg-board-light transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mark focus-visible:ring-offset-2">
                        Booking Sekarang
                    </a>
                <?php else: ?>
                ...
                <a href="<?php echo e(route('marketplace')); ?>" class="block text-center mt-3 text-sm text-ink-muted hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-board rounded py-1">
                    Kembali ke marketplace
                </a>
            </div>
        </div>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\TemanLes\resources\views/tutors/show.blade.php ENDPATH**/ ?>