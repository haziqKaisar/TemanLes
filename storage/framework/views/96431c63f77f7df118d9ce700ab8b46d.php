            ...
            <div class="flex items-center gap-4 sm:gap-6 text-sm">
                <?php if(auth()->guard()->check()): ?>
                    ...
                    <?php if(auth()->user()->isStudent()): ?>
                        <a href="<?php echo e(route('student.dashboard')); ?>" class="text-ink hover:text-board font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-board rounded px-1.5 py-1">Pesanan Saya</a>
                    <?php elseif(auth()->user()->isTeacher()): ?>
                        <a href="<?php echo e(route('teacher.dashboard')); ?>" class="text-ink hover:text-board font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-board rounded px-1.5 py-1 flex items-center gap-1.5">
                            <span class="w-2 h-2 rounded-full bg-board"></span>
                            Dashboard Guru
                        </a>
                    <?php elseif(auth()->user()->isAdmin()): ?>
                        <a href="<?php echo e(route('admin.dashboard')); ?>" class="text-ink hover:text-board font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-board rounded px-1.5 py-1">Panel Admin</a>
                    <?php endif; ?>
                    <span class="hidden sm:inline text-ink-muted border-l border-line pl-4 font-medium">Halo, <span class="text-ink font-semibold"><?php echo e(auth()->user()->name); ?></span></span>
